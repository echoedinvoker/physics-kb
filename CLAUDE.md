...
3. **識別知識單元並預估數量**：掃描所有頁面，按以下分類列出清單，並用校正公式預估總量：
   - **校正公式**（基於 Ch1 數據）：`預估總量 = (概念初估 × 1.5 + 人物初估 × 2.2 + 公式初估 × 1.25 + 應用初估 × 1.0 + 題目初估 × 2.6 + MOC) × 1.1`
   - 列出「初估 → 校正後預估」讓使用者確認
   - 分類列出：
   - 概念（定律、理論、現象）
   - 人物（科學家及其貢獻）
...
6. **三層生成（Tier 1 → Tier 2 → Tier 3）**：
   - **Tier 1：知識骨架**（concepts + scientists + formulas + applications + MOC）
     - 按 section 分批（如 1-1, 1-2, 1-3），每批 ~30 篇，避免撞 context window
     - 一批生成確保 section 內 wikilink 一致性
     - 🆕 新增：按 `templates/` 模板生成，`[[連結]]` 優先指向已存在的筆記
     - 🔄 合併：讀取舊筆記，在對應段落追加新內容，保留舊內容不刪除
   - **Tier 2：題目**
     - 按 section 分批，每批 ~20-30 題
     - prompt 注入 Tier 1 的 note title list（格式見下方）
   - **Tier 3：跨章節連結回填**
     - 新章節全部生成後，跑 `scripts/backfill-links.sh`（見步驟 7）
   - **必須完成 Tier 1 才開始 Tier 2**，確保題目的 `tests_concepts` 連結指向已存在的筆記
...
10. **自動化校驗**：執行 `python3 scripts/verify.py`，檢查 LaTeX、frontmatter、broken links、tag、重複。修復所有 ERROR 再繼續
11. **建索引**：執行 `bash scripts/index.sh`

**Tier 2 Title 注入格式**（生成題目時注入到 system prompt）：

```
## 已存在的筆記（tests_concepts 連結必須從此清單中選取）
- 庫侖定律
- 萬有引力定律
- 牛頓
- ...
...
產生方式：`scripts/export-titles.sh`（Phase 4 建立）或手動列出 Tier 1 生成的 title list。

**筆記生成規則：**

- **嚴格按照 `templates/` 中的模板格式**——模板中的每個段落都是必填，不得省略：
  - **公式筆記**：必須包含「公式」「變數定義」「物理意義」「推導或來源」「應用場景」「計算範例」（已知→求→解→答完整步驟）「易錯點」「相關公式」「相關題目」
  - **人物筆記**：frontmatter 必須包含 `lifetime`（出生-死亡年）、`nationality`、`fields`（貢獻領域列表）；正文必須包含「主要貢獻」（按領域分段）「重要著作與實驗」「歷史定位」「相關人物」「相關題目」
  - **概念筆記**：必須包含「背景知識」「詳細說明」「常見錯誤」「相關概念」「相關題目」
- ID 用 `date +%Y%m%d%H%M` 產生，批量生成時遞增分鐘數
- **context_header 生成**：每篇筆記的 `context_header` 欄位用以下格式填寫——「這是{grade}{subject}第{chapter}章「{section_name}」中關於{topic}的{type}筆記。{one_sentence_summary}。」。範例：「這是高一物理第一章「物理學簡史」中關於天文學革命的概念筆記。本筆記討論哥白尼的日心說如何取代托勒密的地心說，推動科學革命。」context_header 與筆記正文同批生成，不額外跑 LLM
- 每則筆記至少 3 個 `[[連結]]`，方向包含：向上（prerequisites）、平行（related）、向下（applied_in / tested_by）
- **`[[連結]]` 必須指向實際存在（或本次將新建）的筆記**，不要憑空創造連結目標
- **自足性原則**：引用其他概念時，先用一兩句話解釋概念本身，再附 wikilink。讀者不需打開連結就能理解當前筆記的核心內容
  - ❌「其原理類似 [[全反射]]」
  - ✅「其原理基於全反射——當光從較密介質射向較疏介質，入射角大於臨界角時，光完全反射回原介質（詳見 [[全反射]]）」
- 公式必須三層表達：LaTeX + 純文字描述 + 變數表
- 圖片必須有文字等效描述（qmd 搜尋不到圖片內容）
- 題目答案和解析中的每個概念都要連結到對應的概念筆記
- **題目必須包含完整題目文字**：禁止「見教材第 X 頁」，題目筆記必須獨立可讀
